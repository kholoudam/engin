                ...
                <div class="container-fluid py-1 px-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                            ...
                            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Réparation des pannes</li>
                        </ol>
                        <h6 class="font-weight-bolder mb-0">Réparation des pannes</h6>
                    </nav>
                        ...
                        <div class="card my-4">
                            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div class="shadow-primary border-radius-lg pt-4 pb-3" style="background-color:#e10505;">
                                    <h6 class="text-white text-capitalize ps-3">Réparation de l'engin</h6>
                                </div>
                            </div>
                            <div class="card-body px-0 pb-2">
                                <div class="container">
                                    <div class="row">
                                        <form action="{{ route('Reparation_panne.store', $engin->id) }}" method="POST">
                                            @csrf
                                            <div class="modal-header">
                                                <h5 class="modal-title">{{ $engin->Nom }}</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body">
                                                @foreach($engin->details as $detail)
                                                    <div class="form-group">
                                                        <label for="critere_{{ $detail->critere->id }}">{{ $detail->critere->nom }}</label>
                                                        <input type="checkbox" name="criteria[{{ $detail->critere->id }}]" value="repaired" id="critere_{{ $detail->critere->id }}">
                                                    </div>
                                                @endforeach
                                            </div>
