        ...
        <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 bg-gray-600" id="sidenav-main">
            ...
            <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-white active"  style="background-color:#e10505;" href="{{ route('affectation.index') }}">
                            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
                    ...
                    <div class="col-12">
                        <div class="card my-4">
                            ...
                            <div class="card-body px-0 pb-2">
                                ...
                                <form action="{{ route('affectation.store') }}" method="POST" style="margin-left: 200px;">
                                    @csrf
                                    <input type="hidden" name="demande_id" value="{{ $demande->id }}">
                                    <!-- Iterate over EnginDemandes collection -->
                                    @foreach($EnginDemandes as $enginDemande)
                                        <!-- Check if engin relationship is not null -->
                                        @if($enginDemande->engin)
                                            <h5 style="margin-left: 20px;font-family:Fancy;">
                                                <strong>
                                                    <em>
                                                        {{ $enginDemande->engin->Nom }}
                                                    </em>
                                                </strong>
                                            </h5>
                                        @else
                                            <p>No engin found for this EnginDemande.</p>
                                        @endif
                                    @endforeach
                                    @foreach($criteres as $critere)
                                        <div class="form-group">
                                            <div class="form-check form-check-inline">
                                                <label class="d-block">{{ $critere->nom }}</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="criteres[{{ $critere->id }}]" value="oui" required>
                                                <label class="form-check-label">Oui</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="criteres[{{ $critere->id }}]" value="non" required>
                                                <label class="form-check-label">Non</label>
                                            </div>
                                        </div>
                                    @endforeach
                                    <button type="submit" class="btn btn-primary" style="background-color: #00811e; width: 80px; height: 40px; padding: 4px; margin-left: 120px;  box-shadow: rgba(0, 129, 30, 0.16) 0px 10px 36px 0px, rgba(0, 129, 30, 0.06) 0px 0px 0px 1px;">Valider</button>
                                </form>
                            </div>
                        </div>
                    </div>
