            ...
            <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" data-scroll="true">
                <div class="container-fluid py-1 px-3">
                    <nav aria-label="breadcrumb">
                        ...
                        <h6 class="font-weight-bolder mb-0">Détail de la demande</h6>
                    </nav>
                        ...
                        <div class="card my-4">
                            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div class="shadow-primary border-radius-lg pt-4 pb-3" style="background-color:#e10505;">
                                    <h6 class="text-white text-capitalize ps-3">Modifier demande</h6>
                                </div>
                            </div>
                                ...
                                <form action="{{ route('demandes.update', $demande->id) }}" method="POST">
                                    ...
                                    <div class="row justify-content-between text-left">
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Entité</label>
                                            <input type="text" id="Entite" name="Entite" value="{{ $demande->Entite }}">
                                        </div>
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Poste</label> 
                                            <input type="text" id="Poste" name="Poste" value="{{ $demande->Poste }}">
                                        </div>
                                    </div>
                                    <div class="row justify-content-between text-left">
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Shift</label> 
                                            <input type="text" id="Shift" name="Shift" value="{{ $demande->Shift }}">
                                        </div>
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Famille engin</label>
                                            <select id="FamilleEngin" name="FamilleEngin" class="form-control">
                                                @foreach($engins as $engin)
                                                    <option value="{{ $engin->id }}" {{ $engin->id == $detailDemande->FamilleEngin ? 'selected' : '' }}>{{ $engin->Nom }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row justify-content-between text-left">
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Type engin</label> 
                                            <input type="text" id="TypeEngin" name="TypeEngin" value="{{ $detailDemande->TypeEngin }}">
                                        </div>
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Quantité</label>
                                            <input type="number" id="Quantite" name="Quantite" value="{{ $detailDemande->Quantite }}">
                                        </div>
                                    </div>
                                    <div class="row justify-content-between text-left">
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <label class="form-control-label px-3">Raison de la demande</label> 
                                            <input type="text" id="RaisonDemande" name="RaisonDemande" value="{{ $detailDemande->RaisonDemande }}">
                                        </div>
                                    </div>
                                    <div class="row justify-content-between text-left">
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <button type="submit" class="btn btn-primary btn-block mb-4 mt-3" style="background-color:#00811e; width: 120px; margin-left: 400px;">Modifier</button>
                                        </div>
                                        <div class="form-group col-sm-6 flex-column d-flex">
                                            <a class="btn btn-primary btn-block mb-4 mt-3" style="background-color:#ff7684; width: 120px; height: 40px; margin-left: 4px;" href="javascript:history.back();">
                                                <h6>Annuler</h6>
                                            </a>
                                        </div>
                                    </div>
